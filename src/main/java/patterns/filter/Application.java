...
class Application {
  ...
  public static void main(String[] args) {
    ...
    Predicate<Customer> cityPredicate = customer -> customer.getAddress().getCity().equalsIgnoreCase("Chicago");
    Iterator<Customer> cityIterator = customerCollection.filterIterator(cityPredicate);
    while (cityIterator.hasNext()) {
      System.out.println(cityIterator.next());
    }

    System.out.println("Customers zip code starting with 12: ");
    Predicate<Customer> zipPredicate = customer -> customer.getAddress().getZip().startsWith("12");
    Iterator<Customer> zipIterator = customerCollection.filterIterator(zipPredicate);
    while (zipIterator.hasNext()) {
      System.out.println(zipIterator.next());
    }
  }
}
